(function () {
  ...
  root.innerHTML = `
    ...
    <main>
      ...
      ${data.itinerary ? `<section class="itinerary section"><div class="wrap"><div class="itinerary-heading"><div><span class="eyebrow">Roteiro lado oeste</span><h2>Uma jornada entre <em>manguezais, dunas e lagoas</em></h2></div><p>Uma sequência pensada para viver o litoral oeste de Jericoacoara com calma, paisagem e aventura.</p></div><ol>${data.itinerary.map(function (item, i) { return `<li><span>${String(i + 1).padStart(2, "0")}</span><div><h3>${item[0]}</h3><p>${item[1]}</p></div></li>`; }).join("")}</ol></div></section>` : ""}
