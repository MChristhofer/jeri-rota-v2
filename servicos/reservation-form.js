(function () {
      ...
      <div class="reservation-body">
        ...
        <form id="reservation-form" novalidate>
          <div class="reservation-grid">
            ...
            <div class="reservation-field full"><label style="display:flex;align-items:flex-start;gap:10px;line-height:1.45"><input id="reservation-legal" name="legalConsent" type="checkbox" required style="margin-top:4px"><span>Li e concordo com os <a href="/termos-e-condicoes/" target="_blank" rel="noopener noreferrer">Termos e Condições</a> e com a <a href="/politica-de-privacidade/" target="_blank" rel="noopener noreferrer">Política de Privacidade</a> da Jeri Rota.</span></label><span class="field-error" id="error-legalConsent" aria-live="polite"></span></div>
          </div>
        ...
        <section class="reservation-summary" hidden aria-labelledby="summary-title"><div class="reservation-summary-card"><span class="eyebrow dark">Confira antes de enviar</span><h3 id="summary-title" tabindex="-1">Revise os dados da sua solicitação</h3><p class="reservation-summary-route" data-summary-route></p><dl class="reservation-summary-list" data-summary-list></dl><p style="font-size:.82rem;color:#61707a">Aceite: Termos ${TERMS_VERSION} · Privacidade ${PRIVACY_VERSION}</p></div><p class="reservation-status">Solicitação aguardando confirmação da equipe.</p><div class="reservation-actions"><button class="reservation-button secondary" type="button" data-edit-reservation>Voltar e editar</button><a class="reservation-button whatsapp" href="#" target="_blank" rel="noopener noreferrer" data-service="transfer-4x4" data-action="whatsapp-reservation">Enviar solicitação pelo WhatsApp</a></div></section>
      </div>
    </section>`;
  ...
  function openModal() { lastFocus = document.activeElement; modal.hidden = false; document.body.classList.add("reservation-open"); modal.querySelector("#reservation-name").focus(); track("ReservationStart"); }
  function closeModal() { modal.hidden = true; document.body.classList.remove("reservation-open"); if (lastFocus) lastFocus.focus(); }
  ...
  function validate() {
    ...
    var legalAccepted = form.elements.legalConsent.checked;
    setError("legalConsent", legalAccepted ? "" : "Aceite os Termos e a Política de Privacidade para continuar.");
    if (!legalAccepted && !firstInvalid) firstInvalid = form.elements.legalConsent;
    if (firstInvalid) { firstInvalid.focus(); return false; }
    return true;
  }
  function values() { var data = Object.fromEntries(new FormData(form).entries()); Object.keys(data).forEach(function (key) { if (typeof data[key] === "string") data[key] = data[key].trim(); }); return data; }
  function escapeHtml(value) { return String(value).replace(/[&<>"]/g, function (character) { return { "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;" }[character]; }); }
  function formatDate(value) { var parts = value.split("-"); return parts[2] + "/" + parts[1] + "/" + parts[0]; }
  function recordAcceptance() {
    try { sessionStorage.setItem("jeriRotaLegalAcceptance", JSON.stringify({ termsVersion: TERMS_VERSION, privacyVersion: PRIVACY_VERSION, acceptedAt: new Date().toISOString(), source: "transfer-4x4-reservation" })); } catch (e) {}
  }
  ...
  function editReservation() { summary.hidden = true; form.hidden = false; modal.querySelector('[data-step-indicator="summary"]').classList.remove("is-active"); modal.querySelector('[data-step-indicator="form"]').classList.add("is-active"); form.elements.name.focus(); }
  function whatsappMessage() {
    ...
    lines.push("", "✅ Aceite: Termos " + TERMS_VERSION + " e Política de Privacidade " + PRIVACY_VERSION + ".");
  ...
  modal.querySelector('[data-action="whatsapp-reservation"]').addEventListener("click", function (event) { if (!validate()) { event.preventDefault(); editReservation(); return; } recordAcceptance(); track("WhatsAppReservation", { service: "transfer-4x4" }); });
  document.addEventListener("keydown", function (event) {
    if (modal.hidden) return;
    if (event.key === "Escape") closeModal();
    if (event.key === "Tab") {
      var focusable = Array.from(modal.querySelectorAll('button:not([hidden]),input:not([hidden]),textarea:not([hidden]),a[href]')).filter(function (el) { return el.offsetParent !== null; });
